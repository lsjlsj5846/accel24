                                  ...
                                  char *output, float *input)
{
  ...
  for (int i = 0; i < NUM_CHAR; i++)
  {
    ...
    {
      output[tid * (MAX_LEN + 1) + ll] = (char)i;
      // output[ll * N + tid] = (char)i;
      input[tid] = (float)i;
      return;
    }
  }
  output[tid * (MAX_LEN + 1) + ll] = (char)(NUM_CHAR - 1);
  // output[ll * N + tid] = (char)(NUM_CHAR - 1);
...
void namegen(int N, float *random_floats, char *output)
{
  ...
  //   {
  //     float *_mat_to_print;
  //     size_t _to_print_size = N * (1);
  //     CHECK_CUDA(cudaMallocHost(&_mat_to_print, sizeof(float) * _to_print_size));
  //     CHECK_CUDA(cudaMemcpy(_mat_to_print, input->buf_gpu,
  ...
  //         printf("m[%d][%d] = %c", _j, l, (int)(_mat_to_print[_i * N +_j]+0.00001));
  //       }
  //       printf("\n");
  //     }
  //     // exit(0);
  ...
  //   {
  //     char *_mat_to_print;
  //     size_t _to_print_size = N * (MAX_LEN + 1);
  //     CHECK_CUDA(cudaMallocHost(&_mat_to_print, sizeof(char) * _to_print_size));
  //     CHECK_CUDA(cudaMemcpy(_mat_to_print, g_output,
  //                           sizeof(char) * _to_print_size, cudaMemcpyDeviceToHost));
  //     printf("\n");
  //     for (int _j = 10; _j < 15; _j++)
  //     {
  //       for (int _i = 0; _i < 1; _i++)
  //       {
  //         // printf("%f, ", _mat_to_print[_i * N + _j]);
  //         printf("m[%d][%d] = %c", _j, l, _mat_to_print[l * N + _j]);
  //       }
  //       printf("\n");
  //     }
  //     // exit(0);
  ...
  //   for(int _j=0; _j < MAX_LEN+1; _j++){
  //     printf("Output[%d][%d]: %c\n", _i, _j, output[_j * N + _i]);
  //   }
  //   printf("\n");
  // }
  cudaDeviceSynchronize();
